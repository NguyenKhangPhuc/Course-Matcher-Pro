...
export function StorageCard({ count }: StorageCardProps) {
    ...
    return (
        <div className="bg-white rounded-2xl border border-[#d6edf5] p-4 flex flex-col gap-2 shadow-sm">
            <p className="text-[11px] font-semibold text-[#7aa5b0] uppercase tracking-wide">{t.sourceManagement.storageUsed}</p>
            ...
            <p className="text-xs text-[#7aa5b0]">{count.toLocaleString()} / 5,000 {t.sourceManagement.recordsSuffix}</p>
        </div>
    );
}
